...
| Method | Description |
|---|---|
| `getDailySummaries(ctx, { userId, provider?, category, startDate, endDate, maxRows? })` | Get daily summary rows, optionally capped at 1–1000 |
...
| Method | Description |
|---|---|
| `configureOutgoingWebhooks(ctx, args)` | Opt into capture, callbacks, external delivery with `hostActionHandle`, payload modes, and limits |
